"""Network creation module for GKE cluster deployment"""
...
def create_network(
    region: str, app_name: str
) -> Tuple[gcp.compute.Network, gcp.compute.Subnetwork, gcp.compute.Router, gcp.compute.RouterNat]:
    """
    Create VPC network, subnet, router, and NAT for GKE cluster.

    Args:
        region: GCP region for the network resources
        app_name: Application name prefix for resource naming

    Returns:
        Tuple of (network, subnet, router, nat) resources
    """
    # Create VPC network with manual subnet configuration
