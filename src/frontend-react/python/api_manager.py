...
class APIManager:
    ...
    def _load_conversation_history(self, case_id: str) -> List[Dict[str, Any]]:
        ...
        """
        if self.dummy:
            debug_log(f"    [DUMMY MODE] Returning empty conversation for case {case_id}")
            return []
        ...
        if conversation_file.exists():
            try:
                ...
                debug_log(f"Warning: Could not parse conversation file for case {case_id}")
                return []
        else:
            debug_log(f"No conversation found for case {case_id}")
            return []
