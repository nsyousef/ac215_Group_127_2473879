...
class TestPredictionFlow:
    ...
    def test_prediction_request(self, mock_requests):
        ...
        response = mock_requests.post(
            f"{api_manager.PREDICTION_URL}",
            json={"image_embedding": [0.1] * 512}
        )
...
class TestLLMIntegration:
    ...
    def test_llm_explain_request(self, mock_requests):
        ...
        payload = {
            "predictions": {"eczema": 0.78},
            "metadata": {"user_input": "red patch"}
        }
        
        response = mock_requests.post(
            f"{api_manager.DEFAULT_LLM_EXPLAIN_URL}",
            json=payload
        )
    ...
    def test_llm_followup_request(self, mock_requests):
        ...
        response = mock_requests.post(
            f"{api_manager.DEFAULT_LLM_FOLLOWUP_URL}",
            json=payload
        )
