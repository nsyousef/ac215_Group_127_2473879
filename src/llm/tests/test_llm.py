...
from llm import LLM
from mock_data import MOCK_PREDICTIONS, MOCK_METADATA, MOCK_ANSWER1, MOCK_QUESTION1, MOCK_QUESTION2, BASE_PROMPT, QUESTION_PROMPT
...
class TestGeneration:
    ...
    def test_generate_filters_thoughts(self, mock_llm, mock_processor):
        ...
        mock_processor.decode.return_value = "<thought>internal reasoning</thought> This is the actual response about eczema treatment."
        ...
        assert "<thought>" not in response.lower(), \
            "Response should not contain '<thought>' tags"
        assert "</thought>" not in response.lower(), \
            "Response should not contain '</thought>' tags"
        # Check that the actual response content is present
        assert "eczema" in response.lower() or "treatment" in response.lower(), \
...
class TestAskFollowup:
    ...
    def test_ask_followup_updates_history(self, mock_llm):
        ...
        assert len(result["conversation_history"]) == 2, \
            "History should include previous question and new question"
        assert MOCK_QUESTION1 in result["conversation_history"], \
    ...
    def test_ask_followup_empty_history(self, mock_llm):
        ...
        assert len(result["conversation_history"]) == 1, \
            "History should contain just the new question"
        assert result["conversation_history"][0] == MOCK_QUESTION1, \
            "History should contain the question"
