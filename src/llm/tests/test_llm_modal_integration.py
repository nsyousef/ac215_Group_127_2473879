...
from test_llm import MOCK_PREDICTIONS, MOCK_METADATA, MOCK_ANSWER1, MOCK_QUESTION1, MOCK_QUESTION2
...
class TestExplainEndpoint:
    ...
    def test_explain_minimal_data(self):
        ...
        assert response.status_code == 200, \
            f"Expected status 200 even with minimal data, got {response.status_code}"
...
class TestAskFollowupEndpoint:
    ...
    def test_ask_followup_successful_response(self):
        ...
        assert len(data["conversation_history"]) > 0, \
            "Conversation history should contain the question"
    ...
    def test_ask_followup_with_history(self):
        ...
        assert payload["question"] in data["conversation_history"], \
            "New question should be in conversation history"
    ...
    def test_ask_followup_no_history(self):
        ...
        assert len(data["conversation_history"]) >= 1, \
            "History should at least contain the new question"
    ...
    def test_ask_followup_multiple_questions(self):
        ...
        assert payload1["question"] in data2["conversation_history"], \
            "First question should be in history"
        assert payload2["question"] in data2["conversation_history"], \
            "Second question should be in history"
