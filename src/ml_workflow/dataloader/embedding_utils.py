...
def compute_embeddings_and_save(
    data: pd.DataFrame,
    path: str,
    model_name: str,
    batch_size: int,
    ...
    embeddings = encode_with_transformers(
        model_name, texts, batch_size, max_length, device, pooling_strategy, qwen_instr
    )
...
def load_embeddings(
    file: str, image_ids: list[str], text_descs: list[str], on_not_found: Literal["warn", "error"] = "error"
):
    ...
    if not query_img_ids.issubset(existing_img_ids):
        ...
        if on_not_found == "warn":
            logger.warning(
                f"WARNING: The following image_ids were not found in the embedding file: {img_ids_not_found}"
            )
        elif on_not_found == "error":
            raise MissingEmbeddingError(
                f"The following image_ids were not found in the embedding file: {img_ids_not_found}"
            )
        else:
    ...
    if not query_text_desc.issubset(existing_text_desc):
        ...
        if on_not_found == "warn":
            logger.warning(
                f"WARNING: The following text_descs were not found in the embedding file: {text_desc_not_found}"
            )
        elif on_not_found == "error":
            raise MissingEmbeddingError(
                f"The following text_descs were not found in the embedding file: {text_desc_not_found}"
            )
        else:
            raise ValueError(f"Invalid setting for on_not_found: {on_not_found}")
