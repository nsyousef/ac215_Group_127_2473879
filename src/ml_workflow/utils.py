...
def stratified_split(
    ...
    if val_size is None:
        ...
        if train_classes_removed > 0:
            logger.warning(
                f"Filtering out {train_classes_removed} classes from train set with < 2 samples (cannot be stratified for val split)"
            )
...
def save_checkpoint(
    ...
    if model is not None:
        ...
        raise ValueError(
            "Either model, or vision_model with multimodal_classifier, or vision_model with images_classifier must be provided"
        )
