...
def evaluate_model(config_path, save_plots=True, plot_dir="./plots"):
    ...
    precision, recall, f1, support = precision_recall_fscore_support(targets, predictions, average="weighted")
    ...
    for i, class_name in enumerate(class_names):
        print(
            f"{class_name:<20} {precision_per_class[i]:<10.4f} {recall_per_class[i]:<10.4f} "
    ...
    if save_plots:
        create_evaluation_plots(
            targets, predictions, class_names, precision_per_class, recall_per_class, f1_per_class, plot_dir
        )
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/config.yaml", help="Path to configuration YAML file")
    parser.add_argument("--no-plots", action="store_true", help="Disable saving evaluation plots")
    parser.add_argument("--plot-dir", type=str, default="./plots", help="Directory to save evaluation plots")

    args = parser.parse_args()

    # Run evaluation
    results = evaluate_model(config_path=args.config, save_plots=not args.no_plots, plot_dir=args.plot_dir)
