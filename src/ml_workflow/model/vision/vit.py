...
class VisionTransformer(nn.Module):
    ...
    def _get_model(self, model_name: str, pretrained: bool) -> nn.Module:
        ...
        if model_name == "vit_b_16":
            ...
            raise ValueError(
                f"Unsupported ViT model: {model_name}. Supported models: vit_b_16, vit_b_32, vit_l_16, vit_l_32"
            )
        return model
    ...
    def _freeze_layers(self, warmup_mode: bool = False):
        ...
        if self.unfreeze_layers > total_blocks:
            logger.warning(
                f"unfreeze_layers ({self.unfreeze_layers}) > total blocks ({total_blocks}). Unfreezing all blocks."
            )
